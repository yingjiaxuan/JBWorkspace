...
def func_parallel_processor():
    if __name__ == '__main__':  # 并行化模块
        pkuseg.test('C:\Personal_File\DiskF\GSK_Intern\Gsk_Inf\Address_1.txt',
...
# with open('C:\Personal_File\DiskF\GSK_Intern\Gsk_Inf\output.txt', encoding='UTF-8-sig') as f:  # 注意sig
#     line = f.readline()
#     while line:
#         text = re.split(r'[\s\n]', line)
#         text.pop()
#         for i in range(len(text)):
#             tem = re.search(r'/',text[i]).span()
#             text[i]=(text[i][0:tem[0]],text[i][tem[1]:len(text[i])])
#         print('')
#         print(text)
#         line = f.readline()
